...
- Special case of IG is when both $\alpha$ and $\beta$ are zero, which gives something that's not really an IG. But this is the standard non-informative prior. **Jeffrey's default prior for variance of normal distribution**, and it is an improper prior.
...
- Default prior, Jeffrey's prior ($p(\theta) \propto 1/\theta$), is improper when $\alpha = \beta = 0$ but posterior is still proper.
