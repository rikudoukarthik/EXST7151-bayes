...
- $ f(y|x) = \frac{f(x,y)}{f(y)} = \frac{f(y|x)f(x)}{f(y)} $
...
ggplot(df) +
  ...
  theme_classic()
```
- $\theta$ we estimate has meaning only in specific context of sampling model we choose, so sometimes it's useful to explicitly state model in mind
...
- Difficulties/Criticisms:
    - Specificaiton of prior is critical but difficult
...
- A better non-informative prior for beta distribution (than uniform) is an inverse-U, because in this case the impact of the prior on posterior is even lower.
